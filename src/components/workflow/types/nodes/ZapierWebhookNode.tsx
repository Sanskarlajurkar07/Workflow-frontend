...
import { Settings, Trash2, Copy, Zap } from 'lucide-react';
import { useFlowStore } from '../../../../store/flowStore';

const ZapierWebhookNode: React.FC<NodeProps> = ({ id, data, selected }) => {
    ...
    <div className={`bg-white rounded-lg shadow-md overflow-hidden ${
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-600 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-medium text-white">Zapier Webhook</h3>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          <input
            type="text"
            value={data.params?.nodeName || 'zapier_0'}
            onChange={(e) => updateNodeData(id, { nodeName: e.target.value })}
            className="w-full px-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
          />
        </div>
      ...
      <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
        <div className="flex items-center gap-1">
          <span className={`w-2 h-2 rounded-full ${data.params?.url ? 'bg-orange-500' : 'bg-gray-300'}`} />
...
export default ZapierWebhookNode;